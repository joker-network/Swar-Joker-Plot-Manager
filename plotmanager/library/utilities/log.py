...
def _get_log_splits_for_backend(backend, contents):
    backend_parsers = dict(
        joker=_get_log_splits_for_joker,
...
def _get_log_splits_for_joker(contents):
...
def get_phase_info(*args, backend='joker', **kwargs):
    phase_info_parsers = dict(
        joker=_get_joker_backend_phase_info,
...
def _get_joker_backend_phase_info(contents, view_settings=None, pretty_print=True, **kwargs):
...
def get_progress(line_count, progress_settings, backend='joker'):
