...
def _get_metrics(instrumentation_settings):
    ...
    if instrumentation_settings.get('prometheus_enabled', False) and not PROCESSED:
        ...
        GAUGE_PLOTS_RUNNING = Gauge('joker_running_plots', 'Number of running plots', ['hostname', 'queue'])
        COUNTER_PLOTS_COMPLETED = Counter('joker_completed_plots', 'Total completed plots', ['hostname', 'queue'])
