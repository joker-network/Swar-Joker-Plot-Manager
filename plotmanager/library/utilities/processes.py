...
def get_joker_executable_name(backend):
    executable_name_parsers = dict(
        joker=_get_joker_backend_executable_name,
...
def _get_joker_backend_executable_name():
    return f'joker{".exe" if is_windows() else ""}'


def _get_madmax_backend_executable_name():
    return f'joker_plot{".exe" if is_windows() else ""}'


def get_plot_k_size(commands, backend):
    k_size_parsers = dict(
        joker=_get_joker_backend_plot_k_size,
...
def _get_joker_backend_plot_k_size(commands):
    try:
...
def get_joker_drives():
    drive_stats = {'temp': {}, 'temp2': {}, 'dest': {}}
    joker_executable_name = get_joker_executable_name()
    for process in psutil.process_iter():
        try:
            if joker_executable_name not in process.name() and 'python' not in process.name().lower():
                continue
...
def get_plot_id(file_path=None, contents=None, backend='joker'):
    plot_id_parsers = dict(
        joker=_get_joker_backend_plot_id,
...
def _get_joker_backend_plot_id(file_path=None, contents=None):
    if not contents:
        f = open(file_path, 'r')
        contents = f.read()
        f.close()
...
def get_running_plots(jobs, running_work, instrumentation_settings, backend='joker'):
    joker_processes = []
    logging.info(f'Getting running plots')
    joker_executable_name = get_joker_executable_name(backend)
    for process in psutil.process_iter():
        try:
            if joker_executable_name not in process.name() and 'python' not in process.name().lower():
                continue
            ...
            continue
        try:
            if (('plots' not in process.cmdline() or 'create' not in process.cmdline()) and backend == 'joker') or \
        ...
        if process.parent():
            try:
                ...
                pass
        logging.info(f'Found joker plotting process: {process.pid}')
        datetime_start = datetime.fromtimestamp(process.create_time())
        joker_processes.append([datetime_start, process])
    joker_processes.sort(key=lambda x: (x[0]))

    for datetime_start, process in joker_processes:
