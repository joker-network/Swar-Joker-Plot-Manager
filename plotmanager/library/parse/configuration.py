...
def _get_joker_location(config):
    return config.get('joker_location', 'joker')
...
def _get_backend_settings(config):
    return config.get('backend', 'joker')


def get_config_info():
    config = _get_config()
    joker_location = _get_joker_location(config=config)
    ...
    return joker_location, log_directory, jobs, manager_check_interval, max_concurrent, max_for_phase_1, \
